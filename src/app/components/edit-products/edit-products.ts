...
export class EditProducts implements OnInit {
  ...
  ngOnInit(): void {
    this._route.paramMap.subscribe(params => {
        this.productRoute = params.get('route');
        console.log('product slug (route): ', this.productRoute)
  ...
  loadProductData() {
    if (this.productRoute) {
      this._updateS.getProductByRoute(this.productRoute).subscribe({
        next: (productRes: any) => {
          this.productData = productRes.data;
          this.populateForm(productRes.data);
        },
  ...
  onSubmit() {
    if (this.productRoute && this.reactiveForm.valid) {
      ...
      this._updateS.updateProduct(this.productRoute, formData).subscribe({
        next: (response) => {
